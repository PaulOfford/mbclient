...
class MbDatabase:
    ...
    def __init__(self):
        ...
        logging.info(f"mb_database: Working with database {self.db_file}")
    ...
    def create(self):
        ...
        logging.info("mb_database: Creating the progress table")
        ...
        logging.info("mb_database: Creating the status table")
        cursor.execute("""CREATE TABLE settings (ts float, name text, val text, typ text)""")

        logging.info("mb_database: Loading default settings")
        with db:
            cursor.execute(
        ...
        logging.info("mb_database: Loading default status values")
        with db:
            cursor.execute(
        ...
        logging.info("mb_database: Creating post table")
        ...
        logging.info("mb_database: Creating blog table")
        ...
        logging.info("mb_database: Loading list of demo blogs")
        ...
        logging.info("mb_database: Loading welcome info into the post table")
        ...
        logging.info("mb_database: Closing the database connection")
        db.close()
    ...
    def migrate_from_v1(self):
        ...
        with db:
            ...
            logging.info(f"mb_database: {sql_cmd}")
            cursor.execute(sql_cmd)
            rows = cursor.fetchall()
        ...
        for sql_cmd in f.readlines():
            logging.info(f"mb_database: {sql_cmd[0:-2]}")
            with db:
                cursor.execute(sql_cmd)
