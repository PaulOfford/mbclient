...
MAX_QUEUE_SIZE = 20
f2b_q = Queue(maxsize=MAX_QUEUE_SIZE)
b2f_q = Queue(maxsize=MAX_QUEUE_SIZE)
b2c_q = Queue(maxsize=MAX_QUEUE_SIZE)
c2b_q = Queue(maxsize=MAX_QUEUE_SIZE)
...
def _validate_param_value(param: MessageParameter, value: Any) -> Any:
    ...
    if param is MessageParameter.POST_ID:
        ...
        if isinstance(value, str) and value.strip().isdigit():
            return value.strip()
        raise TypeError(f"Parameter '{param.value}' must be an int or a digit string, got {value!r} ({type(value).__name__}).")
...
class UnifiedMessage:
    ...
    @classmethod
    def create(cls, *, target: Optional[TargetLike]=None, typ: Optional[TypeLike]=None, verb: Optional[VerbLike]=None, params: Optional[Mapping[MessageParameterKey, Any]]=None, priority: Optional[int]=None, ts: Optional[float]=None, **extra: Any) -> 'UnifiedMessage':
        ...
        """
        m = cls()
        m.set_many(target=target, typ=typ, verb=verb, params=params, priority=priority, ts=ts, **extra)
        return m

    def set_many(self, *, target: Optional[TargetLike]=None, typ: Optional[TypeLike]=None, verb: Optional[VerbLike]=None, params: Optional[Mapping[MessageParameterKey, Any]]=None, priority: Optional[int]=None, ts: Optional[float]=None, **extra: Any) -> None:
