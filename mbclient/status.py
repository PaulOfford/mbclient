...
class Status:
    ...
    def reload_status(self):
        ...
        db_values_list = status_table.select(where=None, order_by=None, desc=False, limit=1, hdr_list=self.status_cols)
    ...
    def set_callsign(self, callsign: str):
        status_table = DbTable('status')
        status_table.update(value_dictionary={'callsign': callsign})
        self.callsign = callsign

    def set_radio_frequency(self, radio_frequency: int):
        status_table = DbTable('status')
        status_table.update(value_dictionary={'radio_frequency': radio_frequency})
        self.radio_frequency = radio_frequency

    def set_user_frequency(self, user_frequency: int):
        status_table = DbTable('status')
        status_table.update(value_dictionary={'user_frequency': user_frequency})
        self.user_frequency = user_frequency

    def set_offset(self, offset: int):
        status_table = DbTable('status')
        status_table.update(value_dictionary={'offset': offset})
        self.offset = offset
    ...
    def get_selected_blog_name() -> str:
        ...
        db_values = post_table.select(where='is_selected = 1', hdr_list=['blog'])
        if len(db_values) == 0:
            return ''
    ...
    def get_selected_blog_frequency() -> int:
        post_table = DbTable('blog')
        db_values = post_table.select(where='is_selected = 1', hdr_list=['frequency'])
        if len(db_values) == 0:
            return 0
    ...
    def get_selected_post(blog: str) -> int:
        ...
        db_values = post_table.select(where=f"blog='{blog}' AND is_selected = 1", hdr_list=['post_id'])
        if len(db_values) == 0:
            return 0
